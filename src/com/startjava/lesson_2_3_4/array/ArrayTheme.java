...
public class ArrayTheme {
    public static void main(String[] args) {
        ...
        for (int i = 1; i < len2 - 1; i++) {
            sum += numbers2[i];
            String answer = numbers2[i] < len2 - 2 ? String.valueOf(numbers2[i]) + "*" : String.valueOf(numbers2[i]);
            System.out.print(answer);
        }
        System.out.println("=" + sum);
        for (int i = 0; i < len2; i++) {
            String tail = numbers2[i] == numbers2[0] || numbers2[i] == numbers2[len2 - 1] ? "Индекс " + i + " соответствует числу " + String.valueOf(numbers2[i]) + " " : "";
